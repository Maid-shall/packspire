...
public sealed partial class PackspireUiFoundation {
 public void PlayFor(ScreenId from,ScreenId to){
  if(skipNextTransition){skipNextTransition=false;return;}
  if(transitionRoot==null||from==to)return;
  if(transitionRoutine!=null)StopCoroutine(transitionRoutine);
  transitionRoutine=StartCoroutine(Play(Resolve(from,to)));
 }
 TabletopTransitionKind Resolve(ScreenId from,ScreenId to){
  if(from==ScreenId.Hub&&to!=ScreenId.Battle)return TabletopTransitionKind.Fade;
  if(to==ScreenId.Hub&&from!=ScreenId.Battle)return TabletopTransitionKind.Fade;
  if(to==ScreenId.GridBoard)return TabletopTransitionKind.ScrollUnfurl;
 ...
 bool IsBook(ScreenId value)=>value==ScreenId.Character||value==ScreenId.Status||value==ScreenId.Vault||value==ScreenId.Heirloom||value==ScreenId.Reward||value==ScreenId.Shop||value==ScreenId.Event||value==ScreenId.Compendium||value==ScreenId.GameOver||value==ScreenId.GameClear;
