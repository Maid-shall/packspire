...
[Serializable] public class MetaSave { public int version=18,baseGold=24,vaultLevel,runs,wins,dungeonsUnlocked=1,body,hair,consumableCapacity=5; public bool characterMade; public string selectedCharacterId="ren",currentRole="warrior",currentFaction="iron",selectedBackpack="standard",selectedHeirloomUid="",selectedLoadoutId="loadout-1"; public List<ItemInstance> stash=new(); public List<LoadoutSave> loadouts=new(); public List<string> consumables=new(){"heal","heal","guard","fire","energy"}; public List<string> dungeonDiscoveries=new(); public List<string> unlockedRoles=new(){"warrior","guardian","scout","artificer"}; public List<string> discoveredItems=new(){"sword","shield","ember","herb"}; public List<string> discoveredEnemies=new(); public List<string> unlockedSecrets=new(); public List<IdInt> jobLevels=new(){new("warrior",1)}; public List<IdFloat> factionRep=new(){new("iron",0),new("spore",0),new("guild",0),new("void",0)}; public List<ReactionValueState> memoryReactions=new(); }
