...
public enum ScreenId { Character, Hub, Status, Vault, Heirloom, Faction, Expedition, Pack, GridBoard, Battle, Reward, Shop, Event, Compendium, GameOver, GameClear }
...
[Serializable] public class ItemDef { public string id,name,description,cardId,linkRule,explorationCardId; public string[] cardIds; public ItemType type; public CellDef[] cells; public int handle; public Sprite artwork; public ItemDef(string id,string name,ItemType type,string card,string description,params CellDef[] cells){this.id=id;this.name=name;this.type=type;cardId=card;cardIds=(id=="sword"||id=="shield"||id=="dagger")?new[]{card,card}:new[]{card};this.description=description;this.cells=cells;handle=0;} }
[Serializable] public class CardDef { public string id,name,text; public CardType type; public int cost,damage,block,heal,buff,energy,selfDamage; public bool exhaust; public Sprite artwork; public CardDef(string id,string name,CardType type,int cost,string text){this.id=id;this.name=name;this.type=type;this.cost=cost;this.text=text;} }
[Serializable] public class ExplorationCardDef { public string id,name,text,place; public int cost; public Sprite artwork; }
...
[Serializable] public class MetaSave { public int version=17,baseGold=24,vaultLevel,runs,wins,dungeonsUnlocked=1,body,hair,consumableCapacity=5; public bool characterMade; public string selectedCharacterId="ren",currentRole="warrior",currentFaction="iron",selectedBackpack="standard",selectedHeirloomUid="",selectedLoadoutId="loadout-1"; public List<ItemInstance> stash=new(); public List<LoadoutSave> loadouts=new(); public List<string> consumables=new(){"heal","heal","guard","fire","energy"}; public List<string> dungeonDiscoveries=new(); public List<string> unlockedRoles=new(){"warrior","guardian","scout","artificer"}; public List<string> discoveredItems=new(){"sword","shield","ember","herb"}; public List<string> discoveredEnemies=new(); public List<string> unlockedSecrets=new(); public List<IdInt> jobLevels=new(){new("warrior",1)}; public List<IdFloat> factionRep=new(){new("iron",0),new("spore",0),new("guild",0),new("void",0)}; }
