...
public static class DungeonSystem {
 ...
 public static bool Move(DungeonMap map,int target){if(!Adjacent(map.current,target))return false;map.current=target;var node=map.nodes[target];node.visited=true;foreach(var n in map.nodes)if(Adjacent(target,n.id))n.revealed=true;map.alert++;return true;}
}
}
